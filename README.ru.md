...
`version` равна 3 начиная с релиза 1.3.0 — тогда появились блоки `view` и
`freeze`; версия 2 была в 1.2.0 (`nf`, `ha`, `va`, `wrap`), версия 1 — в 1.1.x.
...
- **старые файлы открываются как раньше**, со всем содержимым. Ничего не
  ...
  номер версии и два новых пустых блока;
- **любое сохранение пишет версию 3.** Это сделано намеренно, и аргумент каждый
  раз один и тот же: сборка, которая не знает ключа, его **выбрасывает**. Разбор
  листа в 1.2.0 переносит на чистый лист только известные ему ключи и до
  остальных не доходит — то есть, открыв там файл 1.3.0 и сохранив, он молча
  потерял бы сортировку, фильтры и закреплённые области. Версия 3 для той сборки
  «из будущего», и она откроет файл только для чтения. Ровно для этого поле
  версии и существует. То же самое было с 1.1.x и ключами стилей версии 2.

Новые ключи дописываются в конец, а не вставляются по алфавиту: ключи стилей
1.2.0 — после `bd`, блоки 1.3.0 — после `merges`. Файл, перезаписанный этим
релизом, отличается от старого только добавленными строками.
...
npm test             # 163 юнит-теста: формат, стили, маски, CSV, вставки, i18n, сортировка/фильтры/markdown
...
Размер сборки: `main.js` ~620 КБ, `styles.css` ~112 КБ, сетевых запросов в рантайме
...
**Юнит (163):** round-trip разбора и сериализации, побайтовая детерминированность
...
**E2E (325 проверок):** поднимает **отдельный** экземпляр Obsidian со своим
