...
- Клик по галочке пишет `true` или `false` и сохраняется как любая другая
  правка.
- Кнопка — **переключатель**: она подсвечена, пока галочками являются ВСЕ
  выделенные ячейки, и тогда нажатие снимает тип. На смешанном выделении она
  тёмная, первое нажатие делает галочками всё, значит второе снимает их со
  всего. Третьего состояния нет.
- При превращении ячеек В галочки значения не трогаются: колонка из
  `true`/`false` становится колонкой отмеченных и снятых галочек.
- Когда галочка СНИМАЕТСЯ, вместе с типом стирается и значение, ровно как в
  Google Таблицах. Булево лежало там ради самой галочки; ячейка, которая его
  сохранила бы, показывала бы голое слово `false` и оставалась в файле как
  содержимое, которого никто не вводил. Единственное, что так не стирается, —
  формула.
- **Delete** (и Backspace) на галочке очищает ячейку целиком — и значение, и
  тип, — оставляя обычную пустую ячейку, а не снятую галочку. Заливка, границы
  и числовая маска остаются: Delete чистит содержимое, а не оформление.
- Всё это одна ступень истории документа: Ctrl+Z возвращает галочку вместе с её
  значением.
...
npm test             # 303 юнит-теста: формат, стили, маски, CSV, вставки, i18n,
...
**E2E (948 проверок):** поднимает **отдельный** экземпляр Obsidian со своим
...
значение потом вычитывается **из файла**; та же галочка снимается кнопкой панели,
и проверяется, что в файле не осталось ни типа, ни значения (а один Ctrl+Z
возвращает и то и другое побайтово); на `[[ссылку]]` наводят курсор, чтобы
...
**Тип ячейки снимается так же, как ставился, — вместе со значением.** Убрать
`"t": "cb"` и оставить значение — это и есть то, как «галочка не убирается»
выглядело со стороны пользователя: рамка пропадала, булево, ради которого она
рисовалась, оставалось, и в ячейке торчало голое слово `false`, а в файле —
`{ "v": false }`, то есть содержимое, которого никто не вводил. У Delete была
зеркальная беда: он чистил значение и оставлял тип, поэтому очищенная галочка
тут же возвращалась снятой. Теперь тип и лежащее под ним булево снимаются
вместе (`setCellType(refs, null, true)`), а Delete убирает и тип: он чистит
СОДЕРЖИМОЕ, а галочка — это содержимое, а не оформление. Два места, где тип
снимается попутно (вставка обычных ячеек поверх колонки галочек, протяжка вниз),
сохраняют только что записанные значения, и формулу не стирает ни одно из них.

**У кнопки-переключателя подсветка и направление — один и тот же вопрос.**
Кнопка-галочка загоралась по ПЕРВОЙ ячейке выделения, а нажатие спрашивало
«есть ли среди них не-галочка», поэтому на смешанном диапазоне подсвеченная
кнопка добавляла ещё галочек, и предсказать направление было нельзя. Теперь оба
идут через `isAllCheckbox()` из format.ts: подсвечена ровно тогда, когда тип уже
у всех выделенных ячеек, то есть ровно тогда, когда нажатие его снимет. Одно
нажатие на любом выделении делает галочками всё, следующее снимает всё, третьего
состояния нет.
