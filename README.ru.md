...
| Контекстное меню | Правый клик или долгое нажатие на планшете: правка, копирование, вставка, вставка и удаление строк и колонок, объединение |
...
`.lsheet`) пишет `имя.xlsx` рядом с таблицей и говорит, куда именно. Уезжают
значения, формулы, жирность, кегль, заливка, границы, форматы чисел,
выравнивание, перенос текста, ширины колонок, высоты строк, объединения и по
одному листу на страницу. Существующий `имя.xlsx` заменяется: это экспорт именно
этой таблицы, и два экспорта подряд должны оставлять один файл, а не кучу с
номерами. Экспорт открытого во вкладке файла экспортирует то, что **во
...
**Юнит (200):** round-trip разбора и сериализации, побайтовая детерминированность
...
**E2E (665 проверок):** поднимает **отдельный** экземпляр Obsidian со своим
...
документ называется явно. Одна дыра осталась, и она вендорская: его `setEvents()`
вешает все указательные обработчики на переданный `root`, а `keydown` — всё
равно на глобальный `document`. Поэтому в pop-out стрелки не двигают выделение,
а набор текста прямо по сетке ничего не пишет. Замерено: клик выделяет, панель и
меню работают, двойной клик открывает редактор ячейки, а наша собственная строка
формул (её слушатели висят на её же поле ввода) значение записывает — то есть
pop-out пригоден к работе, просто не с клавиатуры по самой сетке.
...
«Тестируется» теперь включает и e2e, причём это шлагбаум: workflow скачивает
